...
class MCAPI {
   ...
   *
   * @param $schedule_time
   *  The time to schedule the campaign. For A/B Split "schedule" campaigns,
   *  the time for Group A - in YYYY-MM-DD HH:II:SS format in
   *  <strong>GMT</strong>.
   * @param $schedule_time_b Optional
   *  The time to schedule Group B of an A/B Split "schedule" campaign - in
   ...
   * @param $send_type
   *  Optional by default (null) both formats are sent - "html" or "text" send
   *  just that format.
   *
   * @return
   *  True on success.
...
/**
 * Allows one to test their segmentation rules before creating a campaign
 * using them.
 *
 * @section Campaign  Related
 ...
 *  "match" controls whether to use AND or OR when applying your options
 *  expects "<strong>any</strong>" (for OR) or "<strong>all</strong>" (for AND)
 *  "conditions" - up to 10 different criteria to apply while segmenting. Each
 *  criteria row must contain 3 keys - "<strong>field</strong>",
 *  "<strong>op</strong>", and "<strong>value</strong>" - and possibly a fourth,
 *  "<strong>extra</strong>", based on these definitions:
 *
 *    Field = "<strong>date</strong>" : Select based on signup date
 *    Valid Op(eration):
 *      <strong>eq</strong> (is)
 *      <strong>gt</strong> (after)
 *      <strong>lt</strong> (before)
 *    Valid Values:
 *      last_campaign_sent  uses the date of the last campaign sent
 *      campaign_id - uses the send date of the campaign that carriers the Id
 *      submitted - see campaigns()
 *      YYYY-MM-DD - any date in the form of YYYY-MM-DD - <em>note:</em>
 *      anything that appears to start with YYYY will be treated as a date
 *
 *    Field = "<strong>interests-X</strong>": where X is the Grouping Id from l
 *      istInterestGroupings()
 *    Valid Op(erations):
 *      <strong>one</strong>
 *      <strong>none</strong>
 *      <strong>all</strong>
 *    Valid Values: a comma delimited of interest groups for the list -
 *      see listInterestGroupings()
 *
 *    Field = "<strong>aim</strong>"
 *    Valid Op(erations):
 *      <strong>open</strong>
 *      <strong>noopen</strong>
 *      <strong>click</strong>
 *      <strong>noclick</strong>
 *    Valid Values: "<strong>any</strong>" or a valid AIM-enabled Campaign that
 *      has been sent
 *
 *    Field = "<strong>rating</strong>" : allows matching based on list member
 *      ratings
 *    Valid Op(erations):
 *      <strong>eq</strong> (=)
 *      <strong>ne</strong> (!=)
 ...
 *
 *    Field = "<strong>ecomm_prod</strong>" or "<strong>ecomm_prod</strong>":
 *      allows matching product and category names from purchases
 *    Valid Op(erations):
 *      <strong>eq</strong> (=)
 *      <strong>ne</strong> (!=)
 *      <strong>gt</strong> (&gt;)
 *      <strong>lt</strong> (&lt;)
 *      <strong>like</strong> (like '%blah%')
 *      <strong>nlike</strong> (not like '%blah%')
 *      <strong>starts</strong> (like 'blah%')
 *      <strong>ends</strong> (like '%blah')
 *    Valid Values: any string
 *
 *    Field = "<strong>ecomm_spent_one</strong>" or
 *      "<strong>ecomm_spent_all</strong>" : allows matching purchase amounts
 *      on a single order or all orders.
 *    Valid Op(erations):
 *      <strong>gt</strong> (&gt;)
 *      <strong>lt</strong> (&lt;)
 ...
 *      <strong>gt</strong> (after)
 *      <strong>lt</strong> (before)
 *    Valid Values: YYYY-MM-DD - any date in the form of YYYY-MM-DD
 *
 *    Field = "<strong>social_gender</strong>": allows matching against the
 *      gender acquired from SocialPro
 *    Valid Op(eration):
 *      <strong>eq</strong> (is)
 *      <strong>ne</strong> (is not)
 *    Valid Values: male, female
 *
 *    Field = "<strong>social_age</strong>": allows matching against the age
 *      acquired from SocialPro
 *    Valid Op(erations):
 *      <strong>eq</strong> (=)
 *      <strong>ne</strong> (!=)
 *      <strong>gt</strong> (&gt;)
 ...
 *
 *    Field = "<strong>social_influence</strong>" : allows matching against the
 *      influence acquired from SocialPro
 *    Valid Op(erations):
 *      <strong>eq</strong> (=)
 *      <strong>ne</strong> (!=)
 *      <strong>gt</strong> (&gt;)
 ...
 *    Valid Op(erations):
 *      <strong>member</strong> (is a member of)
 *      <strong>notmember</strong> (is not a member of)
 *    Valid Values: twitter, facebook, myspace, linkedin, flickr
 *
 *    Field = "<strong>static_segment</strong>":
 *    Valid Op(eration):
 *      <strong>eq</strong> (is in)
 *      <strong>ne</strong> (is not in)
 *    Valid Values: an int - get from listStaticSegments()
 *
 *    Field = An <strong>Address</strong> Merge Var. Use
 *      <strong>Merge0-Merge30</strong> or the <strong>Custom Tag</strong>
 *      you've setup for your merge field - see listMergeVars(). Note, Address
 *      fields can still be used with the default operations below - this
 *      section is broken out solely to highlight the differences in using the
 ...
 *
 *    Default Field = A Merge Var. Use <strong>Merge0-Merge30</strong> or the
 *      <strong>Custom Tag</strong> you've setup for your merge field - see
 *      listMergeVars()
 *    Valid Op(erations):
 *      <strong>eq</strong> (=)
 *      <strong>ne</strong> (!=)
 ...
 */
  ...
  /**
   * Create a new draft campaign to send. You <strong>can not</strong> have more
   * than 32,000 campaigns in your account.
   *
   * @section Campaign  Related
   ...
   *
   * @param $type
   *  The Campaign Type to create - one of "regular", "plaintext", "absplit",
   *  "rss", "trans", "auto".
   *
   * @param $options
   *  a hash of the standard options for this campaign:
   *    - list_id the list to send this campaign to- get lists using lists().
   *    - subject the subject line for your campaign message.
   *    - from_email the From: email address for your campaign message.
   *    - from_name the From: name for your campaign message (not an email
   *    address).
   *    - to_name the To: name recipients will see (not email address).
   *    - template_id optional - use this user-created template to generate the
   *    HTML.
   *    - content of the campaign (takes precendence over other template
   *      options).
   *    - gallery_template_id optional - use a template from the public gallery
   *      tovgenerate the HTML content of the campaign (takes precendence over
   *      base template options).
   *    - base_template_id optional - use this a base/start-from-scratch
   *      template to generate the HTML content of the campaign.
   *    - folder_id optional - automatically file the new campaign in the
   *    - folder_id passed. Get using folders() - note that Campaigns and
   *      Autoresponders have separate folder setupsn.
   *    - tracking optional - set which recipient actions will be tracked, as a
   *      struct of boolean values with the following keys: "opens",
   *      "html_clicks", and "text_clicks".  By default, opens and HTML clicks
   *      will be tracked. Click tracking can not be disabled for Free accounts.
   *    - title optional - an internal name to use for this campaign. By default,
   *      the campaign subject will be used.
   *    - authenticate optional - set to true to enable SenderID, DomainKeys, and
   *      DKIM authentication, defaults to false.
   *    - analytics optional - if provided, use a struct with "service type" as
   *      a key and the "service tag" as a value. For Google, this should be
   *      "google"=>"your_google_analytics_key_here".
   *      Note that only "google" is currently supported - a Google Analytics
   *      tags will be added to all links in the campaign with this string
   *      attached. Others may be added in the future
   *    - auto_footer optional Whether or not we should auto-generate the footer
   *      for your content. Mostly useful for content from URLs or Imports.
   *    - inline_css optional Whether or not css should be automatically inlined
   *      when this campaign is sent, defaults to false.
   *    - generate_text optional Whether of not to auto-generate your Text
   *      content from the HTML content. Note that this will be ignored if the
   *      Text part of the content passed is not empty, defaults to false.
   *    - auto_tweet optional If set, this campaign will be auto-tweeted when it
   *      is sent - defaults to false. Note that if a Twitter account isn't
   *      linked, this will be silently ignored.
   *    - timewarp optional If set, this campaign must be scheduled 24 hours in
   *      advance of sending - default to false. Only valid for "regular"
   *      campaigns and "absplit" campaigns that split on schedule_time.
   *    - ecomm360 optional If set, our
   *      <a href="http://www.mailchimp.com/blog/ecommerce-tracking-plugin/"
   *      target="_blank">Ecomme</a>
   *
